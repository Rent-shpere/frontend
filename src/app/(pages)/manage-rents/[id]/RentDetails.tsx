    ...
    <div className="px-6">
      <div className="flex flex-col gap-5 px-5 pt-5 border-1 border-[#E6E6E6] rounded-[8px]">
        ...
        {data.map((items) => (
          <div
            key={items.id}
            className="pb-[10px]  border-b-1 border-[#E6E6E6]"
          >
            <div className="grid grid-cols-1 gap-y-12 p-6 border-1 border-[#E9EBF3] shadow-[4px_2px_4px_#DFE5F9]  md:grid-cols-3">
              <div>
                <p className="mb-3 text-sm  font-semibold text-[#8A8A8A]">
                  Office Number
                </p>
                <p className="text-black text-xs">{items.OfficeNo}</p>
              </div>
              <div>
                <p className="mb-3 text-sm  font-semibold text-[#8A8A8A]">
                  Tenant Name
                </p>
                <p className="text-black text-xs">{items.TenantName}</p>
              </div>
              <div>
                <p className="mb-3 text-sm  font-semibold text-[#8A8A8A]">
                  Monthly Payment Amount
                </p>
                <p className="text-black text-xs">{items.MonthlyAmount}</p>
              </div>
              <div>
                <p className="mb-3 font-semibold text-[#8A8A8A]">Start Date</p>
                <p className="text-black text-xs">{items.StartDate}</p>
              </div>
              <div>
                <p className="mb-3 text-sm  font-semibold text-[#8A8A8A]">
                  End Date
                </p>
                <p className="text-black text-xs">{items.EndDate}</p>
              </div>
            </div>
          </div>
        ))}
